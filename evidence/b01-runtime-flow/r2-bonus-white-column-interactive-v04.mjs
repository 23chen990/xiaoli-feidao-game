...
const outDir = new URL('./r2-natural/bonus-white-column-interactive-v04/', root);
await mkdir(outDir, { recursive: true });
const scriptPath = new URL('./r2-bonus-white-column-interactive-v04.mjs', root);
...
const observationLimit = { maxWaitCommands: 2, waitGameMs: 350, operatorResponseMs: 60000 };
...
console.log(`[v04] prefix complete; screenshot=${new URL('observation-01-after-prefix.png', outDir).pathname}`);
console.log('[v04] commands: wait <visible reason> | release <visible reason> | stop');

const decisions = [];
const operatorWaitingStartedAt = Date.now();
let waitCount = 0;
...
for await (const line of rl) {
  ...
  if (Date.now() - operatorWaitingStartedAt > observationLimit.operatorResponseMs && command !== 'stop') {
    console.log('[v04] operator response limit reached; send stop');
    continue;
  }
  if (command === 'wait') {
    if (waitCount >= observationLimit.maxWaitCommands) {
      console.log('[v04] WAIT limit reached; send stop');
      continue;
    }
    waitCount += 1;
    await page.waitForTimeout(observationLimit.waitGameMs);
    ...
    console.log(`[v04] WAIT recorded; screenshot=${screenshot}`);
  } else if (command === 'release') {
    if (releaseSent) {
      console.log('[v04] RELEASE already sent; send stop');
      continue;
    }
    ...
    console.log('[v04] RELEASE sent once; screenshot saved; send stop');
  } else if (command === 'stop') {
    break;
  } else {
    console.log('[v04] unknown command; use wait, release, or stop');
  }
}
rl.close();
...
const report = {
  ...
  scriptVersion: 'B01-BONUS-interactive-v0.4-isolated-output',
  ...
  observationLimit, gameContinuesWhileOperatorThinking: true, decisions, finalState, errors,
  ...
  evidence: ['ready.png', 'observation-01-after-prefix.png', ...decisions.flatMap((decision) => [decision.screenshot, ...(decision.releaseAfterScreenshot ? [decision.releaseAfterScreenshot] : [])]).map((path) => path.replace(`${outDir.pathname}`, 'r2-natural/bonus-white-column-interactive-v04/'))],
  missingEvidence: releaseSent ? [] : ['release-before.png', 'release-after.png'],
  classification: releaseSent ? 'PASS_LOCAL_VISIBLE_WAIT_RELEASE' : 'NOT_RUN_VISIBLE_RELEASE',
};
await writeFile(new URL('./r2-bonus-white-column-interactive-v04.json', root), `${JSON.stringify(report, null, 2)}\n`);
await context.close(); await browser.close();
console.log(`[v04] stopped; audit report written=${new URL('./r2-bonus-white-column-interactive-v04.json', root).pathname}`);
